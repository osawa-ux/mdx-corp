...
### 差別化訴求
AI活用環境（**Claude Code / Gemini** は会社が用意し全スタッフが日常業務で使用）を第一級の採用訴求とする。**2026-07-22 施主確認により Claude → Claude Code に格上げ**（careers/index/about/llms.txt を同時更新済み。以後この主語で統一し、片側だけ変えない）。「全スタッフが使用」「会社が用意」は 2026-07-02 に施主が事実確認済み＝掲載可（根拠のない全数量化・貸与主張を書かない原則自体は維持）。
IT業界経験者を必須としつつ、AIツール未経験でも実務の中で習得できる環境を明示する。

### ガード
- 実績数値は実測のみ・時点明記
- スタッフ実名・写真は本人同意必須
- 医療・クリニック・患者表現は不使用（IT/DX企業として表現）※2026-07-22 施主が「維持」と再決定
- **2026-07-22 追加**: SE候補にも訴求するため「技術環境と、私たちが作っているもの」セクション・キャリアパス行・開発業務FAQ を追加。**技術名・設計方針は works.html 記載範囲に限定する**（works に根拠のない技術名を書かない）。**romubase は works と同じく「開発中」表記で統一**（本番稼働と書かない）。AI環境の「会社支給・全員が日常使用」の主語は **Claude / Gemini**（index.html / llms.txt と一致させる）。Claude Code は「開発・自動化の実務で使用」の粒度に留める
