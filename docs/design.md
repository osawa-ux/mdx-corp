...
- [ ] 採用: 試用期間中の給与以外の条件（休日・勤務時間等）が本採用と同一か施主確認（確認後「その他の条件は変わりません」を注記に復活）
- [ ] 採用: JobPosting のリッチリザルト検証（GSC / リッチリザルトテスト）を反映後に実施
- [ ] 採用: 就業規則第20条（医師以外 8:30〜18:00・休憩90分）と careers 掲載の勤務時間（9:00〜18:00・休憩60分）の関係を施主確認（本職種の所定労働条件の正本確認）
